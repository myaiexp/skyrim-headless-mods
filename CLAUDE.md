...
- **Loading is not working.** The tier-move verified that the six DLLs *load* on 1.7.104.
  ...
  **OneClickTravel** (2026-09-03, against `test --vanilla`, `mods/OneClickTravel/oneclick.steps`)
  ...
  `until:log:` gate) and its four UI-overhaul menu variants (2026-09-08, the same
  `replyonlineend.steps` replayed against each variant's own swf — `mods/DBVODialogueTweaks/
  variants/README.md`). AutoFireBow, AutoCastSpell and GhostAllies are still load-only — re-run each
  mod's own in-engine test before claiming otherwise, and note DBVODialogueTweaks' skip /
  interrupt-cut features are also still untested here.
