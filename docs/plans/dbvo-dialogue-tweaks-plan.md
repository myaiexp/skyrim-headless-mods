...
| `mods/DBVODialogueTweaks/build.sh`                         | Reproducible build: ffdec `-importScript src/DialogueMenu.as` into a copy of `stock/dialoguemenu.swf` → `build/Interface/dialoguemenu.swf`. Follows the repo's `./build.sh [--install]` convention (see `mods/GhostAllies/build.sh`). |
