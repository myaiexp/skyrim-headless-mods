...
| `mods/DBVODialogueTweaks/build.sh`                         | Reproducible build: ffdec `-importScript src/DialogueMenu.as` into a copy of `stock/dialoguemenu.swf` → `build/Interface/dialoguemenu.swf`. Mirrors `mods/RapidBowHold/build.sh` conventions. |
