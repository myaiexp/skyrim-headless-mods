...
| `status`             | —                                                                                           | Write a trace line with (a) world-readiness — `inWorld` (hoisted top-level for a cheap grep) plus a `world` block (`inWorld`/`is3DLoaded`/`mainMenu`/`loadingMenu`), where `inWorld` is the **exact gate `exec` uses** (shared `engine::IsInWorld()`), and (b) currently-armed probes. Host pollers (`skytest ready` / `gs_wait_ready`) wait on `inWorld:true` — the ack alone means nothing (it acks at the main menu too). |
| `mcm-list`           | —                                                                                           | Enumerate registered SkyUI MCMs → `{src:"mcm-list", via:"manager"\|"scan"\|"none", count, mods:[{name, script, pages}]}`. Read-only, headless (no menu needed). Needs the probe loaded where **SkyUI** is (full profile); `count:0` when absent. Added by the MCM-reveal feature — `docs/plans/skytest-probe-mcm-reveal-design.md`.                                                                                            |
| `mcm-get`            | `script`, `props:[…]`                                                                        | Read named properties off a config script class (e.g. `AutoFireBowMCM`) → `{src:"mcm-get", script, values:{prop:val}, missing:[…]}`. Scalar props (bool/int/float/string); absent/non-scalar → `missing`. Acks false only when no quest binds `script`. The deterministic, value-level MCM check (replaces pixel-driving the menu). Same design doc as `mcm-list`.                                                             |
