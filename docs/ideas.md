...
## 2026-09-08 — skytest destroys the real load order (the Jun-14 incident, still live)

**Highest-impact open bug in skytest. It ate the full profile's load order again on 2026-09-08.**

`backup_plugins()` (`skytest:424-436`) is meant to never overwrite the good `.bak` with a
skytest-generated profile. It detects one by checking line 1 for `# Generated by skytest`, which
`write_test_plugins()` / `write_vanilla_plugins()` do stamp.

**Skyrim rewrites `Plugins.txt` on launch with its own Bethesda header**, erasing that marker. So
after any test session that actually booted, the live file looks "real" to the guard, and the next
`backup_plugins()` copies the test profile's handful of lines over the good `.bak`. Both copies of
the load order are then gone.

Confirmed twice: `.profiles/Plugins.txt.bricked` (Jun 14 — 52 plugins, only `*DBVODialogueTweaks.esp`
enabled, Bethesda header) and again 2026-09-08 (live file *and* `.bak` both reduced to 3 lines).
The Jun-14 comment in the source describes the incident correctly but the guard it added does not
prevent it.

**A marker the game overwrites cannot be the guard.** Two directions, not exclusive: keep the
profile state outside `Plugins.txt` (a sidecar file skytest owns, or the profile dir itself), or
refuse to back up any file whose plugin set is a strict subset of the existing `.bak` — the real
invariant is "a backup must never shrink the load order".

Recovery for the 2026-09-08 loss, with the derivation and a validated 52-plugin rebuild (staged,
deliberately **not** applied):
`~/Downloads/skyrim-mods/00-docs/overrides/2026-09-08-load-order-recovery/`.

Two things that recovery turned up, both pre-existing and unrelated to skytest:
- `MCMHelper.esp` and `b612.esp` both declare `SkyUI_SE.esp` as a master while sitting **above** it.
  Skyrim silently refuses to load a plugin whose master is beneath it, so those two had quietly not
  been loading.
- The FWMF trio had six plugins appended after it since June, violating the managing repo's own
  "FWMF must be dead last or the map goes blank/purple" rule.
...
- **DBVO 1.x cannot run on game 1.7.104 at all — nothing here can be tested against it until two
  ...
  *"disabled, incompatible with current version of the game"*. ConsoleUtil speaks the player's line;
  ...
  working on 1.7.99" per its page — re-verified 2026-09-08 by reading its source: CLib-NG plugin
  via `add_commonlibsse_plugin`, no version whitelist, so 1.7.104 is fine) and JContainers SE
  **4.3.2** (file text "SKSE 2.3.1 / SSE (SAE) 1.7.104"). **Correction to the 2026-09-07 date
  recorded earlier: the JContainers upload dated 2026-09-07 is a re-upload of the old 4.3.0 build
  for 1.6.1170, filed under OLD_VERSION. The build you want is the 2026-09-04 MAIN file** — sorting
  the file list by date hands you the broken one. Neither installed here, and the Nexus
...
- **A test profile no longer writes crash logs.** `CrashLogger.dll` is listed in
  ...
  **Update 2026-09-08: "find a build for 1.7.104" is a dead end — 1.25.0 already IS that build.**
  Its only feature commit ("support Skyrim AE 1.7.99") bumped its bundled CommonLibSSE-NG to the
  v6.4.0 release commit, and v6.4.0 is exactly the release that merged "support AE 1.7.99 address
  library format 5". Nothing newer exists on Nexus or GitHub, and no upstream issue reports a
  1.7.104 address-library failure. So the `failed to open address library file` we saw needs a
  *local* explanation, not a newer DLL — CrashLogger is unusual in building a `REL::Offset2ID`
  reverse map at init, which needs the versionlib `.bin` for the running runtime. Also note the
  skip entry was justified from a **test-profile** boot only: `CrashLogger.dll` exists solely in
  `.profiles/full`, and it has never been shown to fail there (the 1.7.104 versionlib has been in
  `full` since 2026-09-02). Re-test in `full` before treating it as broken. Backup option if it is:
  **Trainwreck** (nexus:106440) advertises version-independence and does not use the Address Library.
...
- ~~**Skyrim may already be past 1.7.104.**~~ **Resolved 2026-09-08: 1.7.104 IS current.** The
  installed `SkyrimSE.exe` reports `1.7.104.0`, and Steam's `appmanifest_489830.acf` has
  `buildid == TargetBuildID == 24914197` (fully up to date, auto-update on). Dragonborn ReVoiced's
  "1.7.140" claim does not match anything Steam ships. The verified upstream timeline is
  **1.7.99 = 2026-08-20** (SKSE 2.3.0, AddrLib v12) and **1.7.104 = 2026-08-27** (SKSE 2.3.1,
  AddrLib v13); this machine received it 2026-09-01.
- **1.7.99 → 1.7.104 is NOT a second break — don't skip a file because it says "1.7.99".** Both
  1.7 runtimes ship Address Library **format 5**, and v13 covers everything up to 1.7.104. An
  address-library-independent CommonLibSSE-NG plugin built for 1.7.99 loads on 1.7.104 unchanged
  (verified 2026-09-08 by reading several plugins' own version declarations, not assumed — several
  authors shipped builds after 2026-08-27 without ever bumping their "1.7.99" label). The real
  break was 1.6.1170 → 1.7.x. Corollary for our own builds: one CommonLibSSE-NG rebuild covers
  both 1.7 runtimes.
