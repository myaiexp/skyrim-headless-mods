#!/usr/bin/env bash
# Build GhostAllies.dll headlessly (Linux -> Windows) with clang-cl + xwin.
...
if [[ "${1:-}" == "--install" ]]; then
	...
	mkdir -p "$DEST"
	cp -v "$DLL" "$DEST/GhostAllies.dll"
	echo "installed -> $DEST/GhostAllies.dll"
fi
