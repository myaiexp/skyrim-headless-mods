#!/usr/bin/env bash
# Build AutoCastSpell.dll headlessly (Linux -> Windows) with clang-cl + xwin.
...
DLL="$BUILD_DIR/AutoCastSpell.dll"
echo "built: $DLL"
file "$DLL"
...
if [[ "${1:-}" == "--install" ]]; then
	...
	mkdir -p "$DEST"
	cp -v "$DLL" "$DEST/AutoCastSpell.dll"
	echo "installed -> $DEST/AutoCastSpell.dll"
fi
