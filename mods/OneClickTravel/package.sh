...
#
# OneClickTravel is a single DLL with no install choices, so this ships a PLAIN ZIP: the archive
# root maps to Data/ on install, so the one file sits at SKSE/Plugins/OneClickTravel.dll. No FOMOD —
...
cp "$DLL" "$STAGE/SKSE/Plugins/OneClickTravel.dll"
