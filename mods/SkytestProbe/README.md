...
> **Status: working, v0.3.0.** A runtime-armed SKSE C++ **debug toolkit** — not a gameplay
...
| `placeatme`     | `base` (hex base-object FormID), optional `as` (alias name), `d` (forward distance, default `256`), `freeze` (default `true`)                                 | Spawn `base` (an actor base / bound object) `d` units along the **player's forward vector** at the player's elevation, facing back at the player. `freeze` halts its AI (`EnableAI(false)`) so it stays put for a deterministic test while keeping its collision body. `as` registers a **named alias** (`ally`/`enemy`/…) that later commands resolve like `player`/`crosshair`. The spawned FormID is echoed. **`freeze:true` breaks DIALOGUE:** a frozen NPC opens the topic list but never delivers the reply — the click just parks the menu, which looks exactly like a dialogue-mod softlock. Pass `freeze:false` for any conversation test (verified 2026-09-02 while testing DBVO 2: frozen never replied, unfrozen replied in ~5 s). Use `d:110`–`120` too, so the NPC is inside activation range for `drive tap e`. `0x00013BA3` (Hulda) is a good talkable base — an innkeeper, so several player topics with no quest setup. (`src:"placeatme"`)          |
| `make-teammate` | `ref`, optional `on` (default `true`)                                                                                                                         | Set/clear the actor's `kPlayerTeammate` flag (written directly — no `SetPlayerTeammate` in this CommonLib). `IsPlayerTeammate()` — and so GhostAllies' `IsGhostAlly` — read this bit, so it enrolls a spawned actor as a "ghost ally" without a real follow relationship. (`src:"make-teammate"`)                                                                                                                                            |
| `cast`          | `spell` (hex FormID), optional `ref` (caster, default `player`), `target` (ref; omit for a free cast along facing), `hand` (`right`\|`left`, default `right`) | Cast `spell` from the caster via `MagicCaster::CastSpellImmediate` — spawns a **real** projectile with no player input. **Aim caveat:** with no crosshair it aims at the target's _origin_ (feet/floor), so precise bolts hit the floor and only AoE spells reliably land; the projectile also drops over distance. Fine for _firing_ a player projectile (e.g. to exercise GhostAllies' stamp); unreliable as a precise hit. (`src:"cast"`) |
...
| `speak-watch`   | optional `prefix` (default `"DBVO/"`), `on` (default `true`)                                                                                                  | **Read-only** observer of the PLAYER's DBVO voice line — the only way to answer "is the player's line still audible?", which no screenshot and no UI state can show. MinHook entry detour on `Actor::SpeakSoundFunction` (Address Library 36541/37542, the same target DBVODialogueTweaks hooks) that copies the `BSSoundHandle` by value and SAMPLES it; it never calls `SetVolume`/`FadeOutAndRelease`, so it measures the mod under test instead of its own interference. Emits `src:"speak"` on transitions only: `start` (with the resolved `path`), `playing`, then `stopped` with `elapsedMs` and `why` (`handle-released` = natural end, `not-playing` = stopped while still held). Sampled at `PollIntervalMs`, so `elapsedMs` is ±one tick (±250 ms by default) — fine for "cut at 600 ms vs ran to 1600 ms", too coarse for millisecond work. **Refuses (`ok:false`) in a profile that also contains DBVODialogueTweaks**: MinHook allows one hook per target, so whichever plugin loads second loses — by design, since the observer exists to watch DBVO *without* our mod present. (`src:"speak"`) |
| `ui-invoke`     | `menu` (CommonLib MENU_NAME), `method` (full AS path), optional `args` (string array)                                                                          | Call an ActionScript method inside an open menu — the direct-call form of the SKSE native `UI.InvokeString`. **What it is for:** a menu-side mod is driven by its own Papyrus, and when that Papyrus cannot run (a dead framework DLL, a mod whose scripts you left out to isolate the C++ half) the swf sits unstimulated and the mod looks broken when it is only unpoked. This supplies exactly the missing call. Args go over as GFx strings; AS2 coerces to number where the callee wants one. Acks a named reason for a closed menu / native menu / bad AS path, never a crash. Used by `mods/DBVODialogueTweaks/replyonlineend.steps` to make DBVO 1.x's `startTopicClickedTimer` call on 1.7.104, where DBVO's own Papyrus cannot load. (`src:"ui-invoke"`) |
| `ui-set`        | `menu`, `path` (full AS path), `value` (number or string)                                                                                                     | Set an AS variable on an open menu — the direct-call form of `UI.SetFloat` / `UI.SetString`, i.e. how an MCM normally pushes a setting into a menu swf. With the mod's Papyrus absent the swf runs on compiled-in defaults, and a test that needs a specific value has no other way to set one. (`src:"ui-set"`) |
| `ui-get`        | `menu`, `path`                                                                                                                                                | Read an AS variable back as a string (numbers stringified, so `15000` reads `"15000.000000"`). The query half — confirm a `ui-set` landed, or read swf state a screenshot cannot show. Backs skytest's `until:uivar:<menu>\|<path>\|<value>` replay gate, which is what makes a menu-side mod **assertable**. (`src:"ui-get"`) |
| `papyrus-call`  | `class`, `function`, optional `args:[…]` (0–6 numbers / bools / strings)                                                                                     | Call a Papyrus **global** (static) function — a mod's own `Global Native` API, e.g. `{"cmd":"papyrus-call","class":"DBVOTweaks","function":"SetPlayerVoiceVolume","args":[2.5]}` — via `VirtualMachine::DispatchStaticCall`. The Papyrus counterpart of `ui-invoke`: it sets a mod's setting in a stage with no SkyUI/MCM, and the console cannot (on 1.7.104 `cgf`/`callglobalfunction` answer `Script command … not found`, verified 2026-09-08). **Arg types are yours to match**: a JSON integral goes over as Int, any other number as Float, so pass `2.0` for a Float parameter and `2` for an Int one — the VM does not coerce, and a mismatch fails silently in `Papyrus.0.log`. Any other JSON element type acks `ok:false` naming the index. **The ack means queued, not done**: the VM runs the call on its own stack, and the `src:"papyrus-call"` line written when it returns (`{class, function, ok:true, result}`, `result` a scalar or `null` for None/object/array) is the completion signal — gate on that, not the ack. `ok:false` acks: `no VM` (pre-load) and `dispatch refused` (class or function not found). Each dispatch is also one line in `SkytestProbe.log`. (`src:"papyrus-call"`) |
...
The staging commands — `give-spell`, `set-av`, `placeatme`, `make-teammate`, `cast` — exist as
**direct-engine-call** commands rather than going through
...
result *observed* or acked, type the console when it just needs the world changed. One thing the
typed console cannot reach on 1.7.104 is a mod's Papyrus API — `cgf`/`callglobalfunction` are not
in its command table — so a global function goes through `papyrus-call` (above).
