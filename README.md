...
| `mods/DBVOResponseGap/` | **Staged (scoped, not started).** Make DBVO's player-line→NPC-reply gap configurable (MCM sliders for pad + wpm) so fast voice packs don't dead-air/overlap. swf (ffdec) + Papyrus + MCM; tier-3 = an SKSE plugin reading real audio duration. Scope in its README. |
