...
**The real load order is guarded by `.profiles/.plugins-generated`, not by a header line.** The
flag is set whenever skytest writes a test or vanilla `Plugins.txt`, and cleared only when
`restore_plugins` puts the real one back. While it is set, no backup is taken, and `play` restores
before launching. Skyrim rewrites `Plugins.txt`'s header on every launch, so the older
`# Generated by skytest` marker vanished and a hand-closed session's test list got backed up over
the real order (lost twice: 2026-06-14 and 2026-09-08). Don't go back to guarding on file contents.
